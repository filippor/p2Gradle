...
public class FrameworkTaskConfigurator {
    ...
    /**
     * @param startBundlesSymbolicNames
     *     list of bundle symbolic name to start
     */
    ...
    /**
     * @param sharedPackages
     *     package shared between framework and gradle
     */
    ...
    public FrameworkTaskConfigurator(final Project project, final Path bundleCache) {
        ...
        Task stopFrameworkTask = rootPrj.getTasks()
            .findByName(FrameworkTaskConfigurator.P2_STOP_FRAMEWORK_TASK);
        ...
        Task startFrameworkTask = rootPrj.getTasks()
            .findByName(FrameworkTaskConfigurator.P2_START_FRAMEWORK_TASK);
                    ...
                    t.doLast(it -> {
                        ...
                        this.p2FrameworkLauncher.executeWithServiceProvider((ServiceProvider sp) -> {
                            sp.getService(P2RepositoryManager.class)
                                .init(this.project.getBuildDir()
                                    .toPath()
                                    .resolve("p2")
                                    .resolve("p2Agent")
                                    .toUri(), this.updateSites,
    ...
    public ConfigurableFileCollection bundles(final boolean transitive, final String... bundles) {
        return this.bundles(transitive, Arrays.stream(bundles)
            .map(s -> s.split(":"))
     ...
     *     artifact repository properties for resolution
     * @param bundles
     *     bundles bundles to resolve
     *
     * @return resolved artifacts
     */
    public ConfigurableFileCollection bundles(final boolean transitive, Map<String, String> targetProperties, final String... bundles) {
        return this.bundles(transitive, targetProperties, Arrays.stream(bundles)
            .map(s -> s.split(":"))
            .map(sa -> new Bundle(sa[0], sa.length > 1 ? new org.osgi.framework.VersionRange(sa[1]) : null))
            .toArray(Bundle[]::new));
    }
    ...
    public ConfigurableFileCollection bundles(final boolean transitive, Map<String, String> targetProperties, final Bundle... bundles) {
        ...
        ResolveTask tmpResolve = (ResolveTask) this.project.getTasks()
            .findByName(name);
        if (tmpResolve == null) {
            tmpResolve = this.project.getTasks()
                .register(name, ResolveTask.class, it -> {
...
//        }
     ...
     *     name of new task
     * @param action
     *     action to configure task
     *
     * @return created task
     */
    public TaskProvider<PublishTask> publishTask(final String name, final Action<PublishTask> action) {
        TaskProvider<PublishTask> publishTask = this.project.getTasks()
            .register(name, PublishTask.class, action);
        publishTask.configure(it -> {
            it.getDependsOn()
                .add(this.startFrameworkTask);
            it.p2FrameworkLauncher = this.p2FrameworkLauncher;
        });
    ...
    private FrameworkLauncher createFrameworkLauncher(final Project project) {
        ...
        if (bundles == null) {
            bundles = project.getRootProject()
                .getConfigurations()
                .create(FrameworkTaskConfigurator.P2_FRAMEWORK_BUNDLES_CONFIG);
            project.getRootProject()
                .getDependencies()
                .add(FrameworkTaskConfigurator.P2_FRAMEWORK_BUNDLES_CONFIG, IT_FILIPPOR_P2_P2IMPL);
        }

        final File frameworkStoragePath = project.getBuildDir()
            .toPath()
            .resolve("p2")
            .resolve("p2Framework")
            .toFile();
    ...
    /**
     * @param updateSites
     *     update site to check when resolve
     */
    ...
    /**
     * @param defaultTargetProperties
     *     targetProperties used for dependency that not specify one
     */
